...
> - `Difficulty` enum は廃止 → `difficulty` フィールドは plain string
> - `to_dict()`/`from_dict()` は不要 → `model_dump()` / `Model(**data)` を使用
> - `UserPrefs.rss_sources` はデフォルト空リスト（Firestore から取得）
> - `UserPrefs.default_difficulty` は必須フィールド
> - `Podcast` に `status: Literal["processing","completed","failed","partial_failed"]` と `error_message` を追加
...
class Article(BaseModel):
    id: str; title: str; url: str; source: str; content: str
...
class Podcast(BaseModel):
    id: str; type: PodcastType; article_ids: list[str]; difficulty: str
    audio_url: str; japanese_intro_text: str; duration_seconds: int
    status: PodcastStatus; error_message: str | None = None
    created_at: datetime; user_id: str
```
