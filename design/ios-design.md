...
| 要素 | 詳細 |
|---|---|
| 記事リスト | List。スコア降順（最大 50 件）。スコアは横バーで可視化 |
| スワイプ操作 | `.swipeActions(.leading)` → Star（黄★）／`.swipeActions(.trailing)` → Dismiss（赤×） |
| タップ | `AppState.articleOpenMode` に従う。`in_app` → `SafariView`（`SFSafariViewController`）を `.sheet`／`external` → `openURL` で外部 Safari |
...
| セクション | 内容 | 保存先 |
|---|---|---|
| **アカウント管理** | `AccountSettingsView`：表示名・パスワード変更・ログアウト・管理者導線・Passkey 登録/一覧/削除 | サーバー（セッション）+ Keychain（トークン） |
...
`APIClient` は `@MainActor` で動作し、ベース URL・API キー・セッショントークンを保持する。全リクエストに基盤の `X-API-Key` を付与し、**ログイン後はさらに `Authorization: Bearer <token>`** を付与する（[ADR-013](../adr/013-session-auth-and-user-management.md)）。テスト可能性のため `URLSessionProtocol` を注入する。エラーは `APIError` 型（`network` / `invalidURL` / `httpError(statusCode, detail)` / `decodingError`）。
