...
実装着手後に生じた、iOS設計書（`docs/design/ios-design.html`）との差分を記録する。状態管理・タブ構成・テスト注入・Deployment Target は本対応で**設計書/ADR-010 を実装に合わせて更新済み**。音声再生は `PodcastViewModel` に内包する構成で実装・マージ済み（#79, ios#28）。
