...
**作成日:** 2026-06-10
**改訂日:** 2026-06-12（①WebUI リスタイル反映。デザイン正本が `docs/design/web-design.html` から `docs/design/app-ui.html` に変更され、§2 スタイリング・§10 画面仕様の一部・§11 デザイン仕様を実装に合わせて更新。経緯と決定は ADR-001〜004（`docs/adr/`）を参照。②Phase 2 第1弾: シークバーの再生済みフィル（§10.3）とテストの型付きモック規約（§12）を追記）
...
- `currentPodcast` が null なら非表示。null でなければ: イントロ先頭 50 文字 + DifficultyBadge + 再生/一時停止 + -15s/+30s + シークバー（`input[type=range]`、`aria-label`。再生済み区間を amber でフィル表示 — 再生率を `--seek-fill` カスタムプロパティとしてインライン付与し CSS が描画。duration 0/NaN 時は 0%、0〜100% に clamp。2026-06-12 追加）+ **音量スライダー（`input[type=range]`・0〜100・`aria-label="音量"`。§9 の `setVolume` に接続。order.md タスク 6 の明示要求）** + 速度セレクタ（8 段階、初期値はデフォルト速度）+ `formatDuration` 形式の時間表示
...
- **型付きモック規約（2026-06-12 追加）**: `vi.mock` したモジュールの関数は `vi.mocked(fn).mockReturnValue({...} as unknown as ReturnType<typeof fn>)` の形で型安全に差し替える（生の `fn.mockReturnValue` は TS2339 になるため禁止）。BFF プロキシのテストで Web 標準 `Request` を `NextRequest` 引数へ渡す場合は `as unknown as NextRequest` キャストヘルパーを使う。`npx tsc --noEmit` のエラー 0 件を維持すること
