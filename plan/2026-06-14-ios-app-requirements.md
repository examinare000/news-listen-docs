...
**ステータス:** Phase 1 完了（Feed / Podcast / 音声プレイヤー / Subscriptions / Settings すべて ios `main` にマージ済み）。Phase 2 進行中。
**最終更新日:** 2026-06-29
**関連:** [iOS設計書](../design/ios-design.md) / ADR [007](../adr/007-ios-direct-backend-access.md)・[008](../adr/008-ios-local-state-persistence.md)・[009](../adr/009-ios-signed-url-refetch.md)・[010](../adr/010-ios-architecture-swiftui-mvvm.md)・[011](../adr/011-podcast-generation-status.md) / [Web フロントエンド設計書](../design/web-design.md)

> **ios `main` マージ状況（2026-06-29 時点）:** 本 Living Doc は Phase 1 着手時点の要件であり、現状は以下が ios `main` に反映済み。
> - **Phase 1 全機能マージ済み**: Feed / Podcast / 音声プレイヤー / Subscriptions / Settings（§6 チェックリスト参照）。
> - **Phase 2 着手分**: バックグラウンド再生・ロック画面/コントロールセンター操作（#79, ios#28）＝**マージ＋実機検証完了**。APNs 通知（#80, ios#29）＝**OPEN（未マージ）**。
> - **本要件の前提を変えた既マージ変更**（恒久ドキュメント反映時に §1.1/§2.2/§2.3/§3.1 を是正する）:
>   1. backend URL / API キーのユーザー入力を廃止し**ビルド時注入（`Secrets.xcconfig`）へ集約**（ios#26）→ §3.1・AC-1/AC-2 の「入力画面」前提が変化。
>   2. **生成ステータスのサーバー同期に追従**（ios#27 / [ADR-011](../adr/011-podcast-generation-status.md)）→ §1.1・§2.2 の「`status` 未含有 / Phase 1 対象外」が解消。
>   3. **パスキー（WebAuthn）ログイン導入**（ios#77）→ §2.3「サインイン基盤スコープ外」から逸脱。
...
`docs/design/ios-design.md` に先行する iOS 設計案が存在するが、**実バックエンドとの契約乖離**がある。本要件・設計はこれを正とし、以下の乖離を是正する。

| ios-design.md の記述 | 実バックエンド | 本要件での扱い |
|---|---|---|
...
- [x] `ios/NewsListenApp/` に Xcode プロジェクト（SwiftUI App）と最小ビルド導入
  - 補足: バックグラウンド再生 capability（`UIBackgroundModes=audio`）は #79（ios#28）で追加済み。
...
- [x] Feed（View / ViewModel / Row / スワイプ / SafariView）+ テスト
- [x] Podcast 一覧・詳細（View / ViewModel / Row）+ テスト
- [x] 音声プレイヤー（再生は `PodcastViewModel` に内包・バックグラウンド・位置ローカル保存・署名URL再取得）+ テスト
- [x] Now Playing / Remote Command（ロック画面操作）— #79（ios#28）でマージ＋実機検証完了
- [x] Subscriptions（一覧・追加・削除・バリデーション・エラー）+ テスト　※ソースの「編集」は未提供（#112 で追補予定）
- [x] Settings（難易度・速度・API 設定・テーマ）+ テスト
- [ ] 仕上げ: 空状態・エラー・トースト相当・ダークモード確認（継続）
- [ ] APNs 通知（#80, ios#29）— OPEN（未マージ）

### 6.1 実装メモ・設計書との差分（要レビュー）

実装着手後に生じた、iOS設計書（`docs/design/ios-design.md`）との差分を記録する。状態管理・タブ構成・テスト注入・Deployment Target は本対応で**設計書/ADR-010 を実装に合わせて更新済み**。音声再生は `PodcastViewModel` に内包する構成で実装・マージ済み（#79, ios#28）。
...
| 音声再生の責務 | `AudioPlayerState`（独立 `@Observable`）を `AppState` 経由で全体共有 | `PodcastViewModel` に再生機能を内包（アプリ全体で共有プレイヤー） | **実装・マージ済み（#79, ios#28）**。バックグラウンド再生・ロック画面/CC操作・割り込み対応を含む。実機検証完了。 |
</content>
