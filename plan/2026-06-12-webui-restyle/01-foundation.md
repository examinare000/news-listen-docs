...
- [x] `npm run test` 全パス（既存テストを 1 件も壊さない）
- [x] `npm run build` 成功（next/font のビルド時取得を含む）
- [x] `npm run dev` でトップページにダークテーマ背景（`#0B0A0E`）とフォントが適用されている
- [x] DevTools で `document.documentElement.dataset.theme` が `dark` または `light` に設定される
