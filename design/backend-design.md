...
| 観点 | 設計 |
|-----|------|
| credential 保存 | Firestore `credentials` コレクション。doc-id = `SHA-256(credential_id)`（平文保存なし）。public_key / sign_count / transports / registered_at / last_used_at / user_id / friendly_name を保持 |
...
Response: [ { "credential_id": "...", "friendly_name": "Alice iPhone", "registered_at": "...", "last_used_at": "..." }, ... ]
...
| `backend/api/routers/auth_passkey.py` | Passkey エンドポイント実装（register/options/verify・login/options/verify・credentials list・delete） |
| `backend/shared/firestore_client.py` | `credentials` / `webauthnChallenges` コレクション操作メソッド追加 |
| `backend/api/dependencies.py` | `issue_session` ヘルパ抽出（認証方式に依存しない） |
| `backend/shared/security.py` | Passkey との session 合流に伴う統一 |
| `backend/shared/audit_logs.py` | Passkey 操作ログ出力（`passkey_register` / `passkey_used` / `passkey_removed`） |
...
| ディレクトリ / ファイル | 責務 |
|----------------------|------|
| shared/models.py | Pydantic モデル: Article / UserPrefs / Recommendation / Podcast / PodcastCache / FeaturedSite / User / Session / AuditLog |
...
| api/dependencies.py | 認証依存: get_current_user / get_user_id / require_admin（ADR-013）／ get_audit_logger（ADR-015） |
| api/audit.py | AuditLogger（監査ログ記録のサービス層・ベストエフォート境界・clock 注入・ADR-015） |
| api/routers/auth.py | POST /auth/login·logout（レートリミット・監査計装）、GET·PATCH /auth/me、POST /auth/password |
| api/routers/admin.py | ユーザー管理 CRUD（/admin/users・require_admin・監査計装）＋ 監査ログ参照（/admin/audit-logs）＋ おすすめサイト CRUD（/admin/featured-sites・X-API-Key） |
