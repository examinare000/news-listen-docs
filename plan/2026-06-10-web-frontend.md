...
> **2026-06-12 注記:** 本計画（T1〜T17）は実装完了済み。その後 `docs/plan/2026-06-12-webui-restyle/` の WebUI リスタイルにより、**§3.5（デザイン方向）とフォント・カラートークンは廃止され、デザイン正本は `docs/design/app-ui.html` に変更された**（Tailwind は最終的に導入せず純 CSS）。§2 の D1〜D10（API 乖離決定）と機能仕様は引き続き有効。現行のデザイン仕様は spec §11 を参照。
