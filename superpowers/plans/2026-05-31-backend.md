...
`gs://audio-news-20260531-podcasts` 作成済み（30日 Lifecycle 設定済み）

- [x] **Step 4: サービスアカウントを作成して権限付与**

`audio-news-sa@audio-news-20260531.iam.gserviceaccount.com` 作成済み

> **注意:** 計画のサービスアカウント名 `podcast-app-sa` ではなく `audio-news-sa` として作成済み。デプロイコマンド（Task 13）では `audio-news-sa` を使用する。

- [x] **Step 5: API キーを Secret Manager に保存**

以下のシークレットが登録済み（名前は計画と異なる）:
- `audio-news-api-key`（計画の `podcast-api-key` に相当）
- `audio-news-gemini-key`（計画の `gemini-api-key` に相当）
- `audio-news-openai-key`（将来用・現在は空）

> **注意:** Task 13 のデプロイコマンドで参照するシークレット名を上記に合わせて読み替える。Artifact Registry も `gcr.io` ではなく `asia-northeast1-docker.pkg.dev/audio-news-20260531/audio-news` を使用。
...
```bash
export PROJECT_ID="audio-news-20260531"
export REGION="asia-northeast1"
export SA="audio-news-sa@${PROJECT_ID}.iam.gserviceaccount.com"
export REGISTRY="${REGION}-docker.pkg.dev/${PROJECT_ID}/audio-news"
...
gcloud run jobs create rss-fetcher-job \
  ...
  --set-secrets GEMINI_API_KEY=audio-news-gemini-key:latest \
...
gcloud run deploy podcast-api \
  ...
  --set-secrets API_KEY=audio-news-api-key:latest,GEMINI_API_KEY=audio-news-gemini-key:latest
...
SA="audio-news-sa@audio-news-20260531.iam.gserviceaccount.com"
PROJECT_ID="audio-news-20260531"
