...
**ステータス:** ドラフト（実装着手前のスコープ確定用 Living Doc）
**最終更新日:** 2026-06-14
**関連:** [設計書](../design/2026-06-14-ios-app-design.md) / ADR [007](../adr/007-ios-direct-backend-access.md)・[008](../adr/008-ios-local-state-persistence.md)・[009](../adr/009-ios-signed-url-refetch.md)・[010](../adr/010-ios-architecture-swiftui-mvvm.md)・[011](../adr/011-podcast-generation-status.md) / [Web フロントエンド仕様書](../spec/2026-06-10-web-frontend-spec.md)
...
- 記事タップで元記事 URL を開く（iOS: `SFSafariViewController`）。
...
| AC-5 | Feed 表示中 | 記事タップ | `SFSafariViewController` で元記事が開く |
...
- [ ] `ios/` に Xcode プロジェクト（SwiftUI App）と最小ビルド導入（バックグラウンド再生 capability 含む）
- [ ] Models（Codable）: Article / Podcast / RssSource — バックエンド契約に一致（snake_case → CodingKeys）
- [ ] APIClient（`URLSessionProtocol` 注入）+ APIError + エンドポイント定義 + テスト
- [ ] AppState（@Observable）+ 初回セットアップ + ローカル永続化
- [ ] Feed（View / ViewModel / Row / スワイプ / SafariView）+ テスト
- [ ] Podcast 一覧・詳細（View / ViewModel / Row）+ テスト
...
- [ ] Settings（難易度・速度・API 設定・テーマ）+ テスト
