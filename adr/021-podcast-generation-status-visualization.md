...
本 ADR 中「iOS の status 消費は ios#15 で今後対応」とした留保について、**ios#27（commit `c50287a`、マージ済み）で実現**した。

- iOS アプリが API の `status` フィールドを消費・表示する機能を実装
- Podcast モデルに `status` フィールドを追加し、サーバー値を反映
- 生成状態をアプリで認識可能（実装詳細は iOS アーキテクチャと合わせ、適切に表示）

なお Web Push（ADR-020）や完全なポーリング UI は引き続き ios#15 スコープとして保留されている。
