...
- `articleOpenMode`（記事の開き方: `in_app` / `external`、既定 `in_app`）
- 音量（@AppStorage）
...
- **FeedView**: `List`、leading=Star / trailing=Dismiss の `swipeActions`、`.refreshable`、`.task` 初回ロード、`ContentUnavailableView` で空/エラー。タップ時は `AppState.articleOpenMode` に従い、`in_app` なら `SafariView`（`SFSafariViewController`）を `.sheet` 表示、`external` なら `openURL`（`UIApplication.shared.open` 相当）で外部起動。スコアは横バーで可視化。
- **PodcastView**: `List`（難易度バッジ・種別・イントロ抜粋・再生時間・続き位置）、タップで再生（§6 フロー）、再生中インジケータ。詳細はサブ画面で全文表示。
- **SettingsView**: RSS ソース管理セクション（一覧 + `onDelete`→確認→削除、追加シート: 名前/URL・クライアントバリデーション・409/422 をインライン表示、成功でシートを閉じ一覧更新）、既定難易度 `Picker`、既定速度 `Slider`/`Picker`、**記事の開き方 `Picker`（アプリ内 / 外部 Safari）**、API 設定（`SecureField` でキー）、テーマ切替。RSS 一覧以外は端末ローカル保存。
