...
本 ADR で留保していた「定時実行時の per-user fan-out」について、backend#56（commit `22cbdbf`）で **action 起動経路の per-run `USER_ID` 伝播が実現**した。

- `shared/job_trigger.py` が v2 Jobs API (`RunJob`) で `overrides.containerOverrides[].env` 経由 `USER_ID` を伝播
- action（Star/Dismiss）直後のジョブ実行は既にユーザー単位で行われている

定時 Scheduler 経路（単一ユーザー全体ジョブ）については、「ユーザー列挙して各 `USER_ID` で起動」の fan-out が引き続き必要。本 ADR では決定論的に「いずれのジョブも同一 `cache_key` の `processing` を共有キャッシュへ確保できる」前提が対象であり、per-user fan-out の実装・検証は Phase 2 スコープのままである。
